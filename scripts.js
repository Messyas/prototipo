...
const state = {
  ...
  actionFilters: { period: 'Agosto/2026', productArea: 'Todos', scrapLine: 'Todas', owner: 'Todos', category4m: 'Todos', risk: 'Todos', status: 'Todos', overdue: 'Todos' },
...
function renderRelativeDashboardAvailable({ filters, metric, masked, dashboardRows, dimensionFactor }) {
  ...
  const rateLabel = metric === 'usd' ? 'Scrap Rate de IF Cost' : 'Scrap Rate por quantidade';
...
function renderRelativeDashboard(params) {
  ...
  return `<section class="relative-unavailable" role="status"><div><span class="panel-kicker">Scrap Rate · eficiência relativa</span><h2>Scrap Rate indisponível</h2><p>${denominatorLabel} ainda não foi recebido para este recorte.</p></div>${badge('Aguardando integração','info')}</section>`;
}
...
function renderDashboard() {
  ...
  const absoluteContent = `<section class="dashboard-kpi-grid dashboard-absolute-kpis">${kpis}</section>
    ...
    ${tablePanel(periodContext.isCurrentDetail ? 'Ocorrências prioritárias' : `Ocorrências prioritárias — ${filters.period}`,[{label:'Data'},{label:'Produto'},{label:'Linha / posto'},{label:'Item / componente'},{label:'QTY',number:true},{label:'IF Cost',number:true},{label:'Revisão'},{label:'Ações'}],attention)}`;
...
function renderReports() {
  ...
  const tabs=`<div class="tabs" role="tablist">${[['construir','Construir relatório'],['versoes','Versões'],['envios','Envios']].map(([id,label])=>`<button class="tab ${state.reportTab===id?'active':''}" data-action="report-tab" data-id="${id}">${label}</button>`).join('')}</div>`;
  let content='';
